...
- 2026-05-11: `cargo test` passed after pivoting the current 2D runtime to import DAT collision geometry into table-local space: the table outline, component `600`/`603` walls, oneway points, `306` circles, and ready-ball feed path now project through the shared DAT/world-to-table-local helpers before entering physics, while `EdgeManager` now also supports non-zero collision-grid origins for projected table bounds.
- 2026-05-11: `cargo test` passed after adding source-backed `proj::z_distance` and DAT `501` depth-sample helpers in `src/assets/group.rs`, then teaching world-plane bitmap rendering to choose a depth-sorted frame before projection so the live ball visual follows the first real `TBall::Repaint` sprite-selection rule.
- 2026-05-11: `cargo test` passed after adding an explicit world-plane bitmap coordinate mode, teaching render to project centered world-space sprite rects through the shared `TableView` helpers, and moving the live ball visual onto that path as the first source-backed `TBall::Repaint` alignment slice.
- 2026-05-11: `cargo test` passed after adding source-backed DAT attribute `601` helpers in `src/assets/group.rs`, documenting it as a world-plane feed point with an explicit `z = 0` table-local projection path, and moving the plunger spawn reader in `table.rs` onto the named helper without changing runtime behavior.
- 2026-05-11: `cargo test` passed after porting decompilation-backed reverse projection (`proj::ReverseXForm`) into `src/assets/group.rs`, exposing table-local-to-world-plane helpers alongside the existing forward projection path, and covering the new bridge with reverse-projection and round-trip tests.
- 2026-05-11: `cargo test` passed after adding shared named/sequence bitmap rect helpers in `src/assets/group.rs`, covering their table-local behavior with focused tests, and switching render sequence placement to use the canonical asset-layer conversion path.
- 2026-05-11: `cargo test` passed after wrapping DAT table-origin and optional projection metadata into a shared `TableView` helper in `src/assets/group.rs`, with tests covering table-local bitmap conversion and decompilation-backed world-to-table-local projection math.
- 2026-05-10: `cargo test` passed after adding source-backed `camera_info`/table-`700` projection helpers in `src/assets/group.rs`, with synthetic tests matching the decompiled `proj.cpp` math and table-local origin subtraction, as the next coordinate-space cleanup slice.
- 2026-05-10: `cargo test` passed after adding shared DAT-to-table-local bitmap rect helpers in `src/assets/group.rs`, switching sequence rendering to use that canonical conversion path, and documenting the current plunger-feed coordinate boundary in `table.rs` as the first coordinate-space cleanup slice.
